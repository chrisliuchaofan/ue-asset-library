# 🔍 服务器诊断和启动指南

## 步骤 1：检查后端服务是否在运行

在服务器终端执行：

```bash
# 检查 3002 端口是否被占用
netstat -tuln | grep 3002
# 或者
lsof -i :3002
# 或者
ss -tuln | grep 3002
```

如果**没有输出**，说明服务没有运行。

## 步骤 2：检查是否有后台进程

```bash
# 检查是否有 node 进程在运行
ps aux | grep node
ps aux | grep "ts-node-dev"
ps aux | grep "nest"
```

## 步骤 3：启动后端服务

### 方法 1：使用启动脚本（推荐）

```bash
cd /opt/ue-assets-backend/backend-api
bash 服务器启动后端服务.sh
```

### 方法 2：手动启动

```bash
cd /opt/ue-assets-backend/backend-api

# 检查环境变量
cat .env | grep PORT

# 启动服务（会在前台运行，可以看到日志）
npm run start:dev
```

### 方法 3：后台运行（使用 nohup）

```bash
cd /opt/ue-assets-backend/backend-api

# 后台运行，日志输出到 nohup.out
nohup npm run start:dev > backend.log 2>&1 &

# 查看进程 ID
echo $!

# 查看日志
tail -f backend.log
```

### 方法 4：使用 screen（推荐用于长期运行）

```bash
# 安装 screen（如果没有）
# yum install screen  # CentOS/RHEL
# apt-get install screen  # Ubuntu/Debian

# 创建新的 screen 会话
screen -S backend

# 在 screen 中启动服务
cd /opt/ue-assets-backend/backend-api
npm run start:dev

# 按 Ctrl+A 然后按 D 来分离会话（服务继续运行）

# 重新连接会话
screen -r backend

# 查看所有会话
screen -ls
```

## 步骤 4：验证服务已启动

服务启动后，应该看到类似输出：

```
[Nest] XXXX  - 12/15/2025, X:XX:XX PM     LOG [NestFactory] Starting Nest application...
[Nest] XXXX  - 12/15/2025, X:XX:XX PM     LOG [InstanceLoader] ...
[Nest] XXXX  - 12/15/2025, X:XX:XX PM     LOG [NestApplication] Nest application successfully started
Backend API running on port 3002
```

然后测试服务：

```bash
# 测试健康检查（如果有的话）
curl http://localhost:3002/health

# 或者测试根路径
curl http://localhost:3002/
```

## 步骤 5：创建用户

服务运行后，执行：

```bash
# 1. 注册 test@factory-buy.com
curl -X POST http://localhost:3002/users/register \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"password123","name":"测试用户"}'

# 2. 注册 admin@admin.local
curl -X POST http://localhost:3002/users/register \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@admin.local","password":"admin123","name":"管理员"}'
```

## ⚠️ 常见问题

### 问题 1：端口被占用

```bash
# 查找占用 3002 端口的进程
lsof -i :3002
# 或者
netstat -tuln | grep 3002

# 杀死进程（替换 PID 为实际进程 ID）
kill -9 <PID>
```

### 问题 2：依赖未安装

```bash
cd /opt/ue-assets-backend/backend-api
npm install
```

### 问题 3：环境变量未配置

```bash
cd /opt/ue-assets-backend/backend-api

# 检查 .env 文件是否存在
ls -la .env

# 查看环境变量
cat .env | grep -E "PORT|DATABASE|OSS"
```

### 问题 4：数据库连接失败

检查：
1. Docker 容器是否运行：`docker ps | grep postgres`
2. 数据库连接配置是否正确：`cat .env | grep DATABASE`
3. 数据库是否可访问：`docker exec -it <postgres_container> psql -U ue_user -d ue_assets`

## 📝 完整操作流程

```bash
# 1. 进入后端目录
cd /opt/ue-assets-backend/backend-api

# 2. 检查服务是否运行
lsof -i :3002

# 3. 如果没有运行，启动服务（选择一种方式）
# 方式 A：前台运行（可以看到日志）
npm run start:dev

# 方式 B：后台运行
nohup npm run start:dev > backend.log 2>&1 &

# 方式 C：使用 screen
screen -S backend
npm run start:dev
# 按 Ctrl+A 然后 D 分离

# 4. 等待服务启动（看到 "Backend API running on port 3002"）

# 5. 测试服务
curl http://localhost:3002/

# 6. 创建用户
curl -X POST http://localhost:3002/users/register \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"password123","name":"测试用户"}'

curl -X POST http://localhost:3002/users/register \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@admin.local","password":"admin123","name":"管理员"}'
```







