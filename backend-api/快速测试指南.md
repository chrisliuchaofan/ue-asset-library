# 🚀 快速测试指南

## 前置条件

1. **后端服务已启动**
   ```bash
   pm2 status
   ```

2. **配置测试用户**

   在服务器上的 `/opt/ue-assets-backend/backend-api/.env` 文件中添加：

   ```env
   USER_WHITELIST=test@factory-buy.com:password123
   INITIAL_CREDITS=100
   JWT_SECRET=your-secret-key-change-in-production
   ```

   然后重启服务：
   ```bash
   pm2 restart ue-assets-backend --update-env
   ```

---

## 方法 1：使用浏览器控制台测试（推荐）

1. **打开前端页面**
   - 访问：`https://www.factory-buy.com`
   - 按 `F12` 打开开发者工具
   - 切换到 `Console` 标签

2. **复制并执行以下代码**

```javascript
// 完整测试流程
(async function() {
  const BACKEND_URL = 'https://api.factory-buy.com';
  const TEST_EMAIL = 'test@factory-buy.com';
  const TEST_PASSWORD = 'password123';
  
  let token = null;
  
  try {
    // 1. 健康检查
    console.log('1️⃣ 测试健康检查...');
    const healthRes = await fetch(`${BACKEND_URL}/health`);
    const healthData = await healthRes.json();
    console.log('✅ 健康检查:', healthData);
    
    // 2. 登录
    console.log('\n2️⃣ 测试登录...');
    const loginRes = await fetch(`${BACKEND_URL}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: TEST_EMAIL, password: TEST_PASSWORD })
    });
    const loginData = await loginRes.json();
    if (loginData.token) {
      token = loginData.token;
      console.log('✅ 登录成功，Token:', token.substring(0, 20) + '...');
    } else {
      console.error('❌ 登录失败:', loginData);
      return;
    }
    
    // 3. 查询积分
    console.log('\n3️⃣ 查询积分余额...');
    const balanceRes = await fetch(`${BACKEND_URL}/credits/balance`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'X-User-Id': TEST_EMAIL
      }
    });
    const balanceData = await balanceRes.json();
    console.log('✅ 积分余额:', balanceData);
    
    // 4. 消费积分
    console.log('\n4️⃣ 消费积分...');
    const consumeRes = await fetch(`${BACKEND_URL}/credits/consume`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
        'X-User-Id': TEST_EMAIL
      },
      body: JSON.stringify({ amount: 5, action: 'test' })
    });
    const consumeData = await consumeRes.json();
    console.log('✅ 消费积分成功:', consumeData);
    
    // 5. 创建日志
    console.log('\n5️⃣ 创建日志...');
    const logRes = await fetch(`${BACKEND_URL}/logs/create`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
        'X-User-Id': TEST_EMAIL
      },
      body: JSON.stringify({
        action: 'test',
        details: { test: true },
        success: true,
        timestamp: new Date().toISOString()
      })
    });
    const logData = await logRes.json();
    console.log('✅ 创建日志成功:', logData);
    
    console.log('\n🎉 所有测试通过！');
  } catch (error) {
    console.error('❌ 测试失败:', error);
  }
})();
```

---

## 方法 2：使用 Node.js 测试脚本

1. **在服务器上执行**

```bash
cd /opt/ue-assets-backend/backend-api

# 设置环境变量
export BACKEND_URL=https://api.factory-buy.com
export TEST_EMAIL=test@factory-buy.com
export TEST_PASSWORD=password123

# 运行测试脚本
node test-integration.js
```

---

## 方法 3：使用 curl 命令测试

```bash
# 1. 健康检查
curl https://api.factory-buy.com/health

# 2. 登录（需要先配置用户）
curl -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"password123"}'

# 3. 查询积分（使用登录获得的 token）
curl https://api.factory-buy.com/credits/balance \
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \
  -H "X-User-Id: test@factory-buy.com"
```

---

## 预期结果

### 健康检查
```json
{
  "status": "ok",
  "timestamp": 1234567890
}
```

### 登录成功
```json
{
  "success": true,
  "userId": "test@factory-buy.com",
  "email": "test@factory-buy.com",
  "name": "test",
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
}
```

### 查询积分
```json
{
  "balance": 100
}
```

### 消费积分
```json
{
  "success": true,
  "balance": 95,
  "transactionId": "txn-1234567890-abc123"
}
```

### 创建日志
```json
{
  "logId": "uuid-12345678-1234-1234-1234-123456789abc"
}
```

---

## 常见问题

### 问题 1：登录失败 "邮箱或密码错误"

**原因：** 用户未配置在 `USER_WHITELIST` 中

**解决：**
1. 检查 `.env` 文件中的 `USER_WHITELIST` 配置
2. 格式：`email:password,email2:password2`
3. 重启服务：`pm2 restart ue-assets-backend --update-env`

### 问题 2：401 未授权

**原因：** Token 无效或过期

**解决：**
1. 重新登录获取新 token
2. 检查 `JWT_SECRET` 环境变量是否正确

### 问题 3：CORS 错误

**原因：** 后端 CORS 配置不正确

**解决：**
1. 检查后端 `main.ts` 中的 CORS 配置
2. 确认 `FRONTEND_URL` 环境变量包含前端域名
3. 重启服务

---

## 下一步

测试通过后，可以：

1. **集成到前端代码**
   - 创建 API 客户端封装
   - 在需要的地方调用后端 API

2. **添加前端 UI**
   - 显示积分余额
   - 显示积分消费记录
   - 显示操作日志

3. **完善功能**
   - 用户注册
   - 密码重置
   - 积分充值


