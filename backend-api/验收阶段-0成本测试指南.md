# 🧪 验收阶段 - 0 成本回归测试指南

## 📋 测试前准备

### 1. 配置 Dry Run 模式

在服务器上的 `.env` 文件中添加：

```env
# Dry Run 模式配置
MODEL_ENABLED=false          # 禁用真实模型调用
BILLING_ENABLED=false        # 禁用真实扣费

# 其他配置保持不变
```

### 2. 重启服务

```bash
pm2 restart ue-assets-backend --update-env
```

---

## ✅ 测试清单

### 测试 1：幂等扣费（数据库级约束）

**目标：** 确保同一个 jobId 只能产生一次扣费交易

**步骤：**

```bash
# 1. 登录获取 token
TOKEN=$(curl -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"password123"}' \
  | jq -r '.token')

# 2. 第一次消费（带 refId）
curl -X POST https://api.factory-buy.com/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "amount": 10,
    "action": "test_action",
    "refId": "job-12345"
  }'

# 3. 第二次消费（相同的 refId + action）- 应该返回幂等结果
curl -X POST https://api.factory-buy.com/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "amount": 10,
    "action": "test_action",
    "refId": "job-12345"
  }'
```

**预期结果：**
- ✅ 第一次：正常扣费，返回 `isIdempotent: false`
- ✅ 第二次：返回已存在的交易，`isIdempotent: true`，余额不变

**验证：**
```bash
# 查询交易记录，应该只有一条
curl https://api.factory-buy.com/credits/balance \
  -H "Authorization: Bearer $TOKEN"
```

---

### 测试 2：Dry Run 模式 - 模型调用

**目标：** 验证 MODEL_ENABLED=false 时不调用真实模型

**步骤：**

```bash
# 1. 调用 AI 生成（Dry Run 模式）
curl -X POST https://api.factory-buy.com/ai/generate-text \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "prompt": "你好，请介绍一下你自己",
    "presetId": "qwen-turbo-standard"
  }'
```

**预期结果：**
- ✅ 返回 mock 数据，包含 `[Mock Response]` 标记
- ✅ `raw.mock: true`
- ✅ 不产生任何模型 API 调用费用

**验证：**
- 检查响应中的 `raw.mock` 字段
- 检查服务器日志，不应该有真实的 API 调用

---

### 测试 3：Dry Run 模式 - 扣费

**目标：** 验证 BILLING_ENABLED=false 时不写真实扣费

**步骤：**

```bash
# 1. 查询当前余额
BALANCE_BEFORE=$(curl https://api.factory-buy.com/credits/balance \
  -H "Authorization: Bearer $TOKEN" | jq -r '.balance')

# 2. 消费积分（Dry Run 模式）
curl -X POST https://api.factory-buy.com/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "amount": 10,
    "action": "test_dry_run"
  }'

# 3. 再次查询余额
BALANCE_AFTER=$(curl https://api.factory-buy.com/credits/balance \
  -H "Authorization: Bearer $TOKEN" | jq -r '.balance')
```

**预期结果：**
- ✅ 返回 `isDryRun: true`
- ✅ 返回模拟的 `transactionId`（以 `mock-txn-` 开头）
- ✅ 余额不变（`BALANCE_BEFORE === BALANCE_AFTER`）
- ✅ 数据库中没有新增交易记录

**验证：**
```bash
# 检查数据库（应该没有新记录）
# 或者检查返回的 isDryRun 字段
```

---

### 测试 4：参数白名单验证

**目标：** 验证生产环境禁止自由指定参数

**步骤：**

```bash
# 1. 获取可用预设列表
curl https://api.factory-buy.com/ai/presets \
  -H "Authorization: Bearer $TOKEN"

# 2. 使用有效预设（应该成功）
curl -X POST https://api.factory-buy.com/ai/generate-text \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "prompt": "测试",
    "presetId": "qwen-turbo-standard"
  }'

# 3. 尝试自由指定参数（生产环境应该失败）
curl -X POST https://api.factory-buy.com/ai/generate-text \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "prompt": "测试",
    "provider": "qwen",
    "model": "qwen-turbo",
    "maxTokens": 5000
  }'
```

**预期结果：**
- ✅ 使用预设：成功
- ✅ 自由指定参数（生产环境）：返回 400 错误，提示必须使用 presetId

**注意：** 开发环境（NODE_ENV !== 'production'）允许自由指定参数，用于测试。

---

### 测试 5：Job 流程完整性（Dry Run）

**目标：** 验证 Dry Run 模式下 Job 流程完整跑通

**步骤：**

```bash
# 1. 创建任务
JOB_ID=$(curl -X POST https://api.factory-buy.com/jobs \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "type": "text_generation",
    "input": {
      "prompt": "测试提示词",
      "presetId": "qwen-turbo-standard"
    }
  }' | jq -r '.id')

# 2. 查询任务状态（应该是 queued）
curl https://api.factory-buy.com/jobs/$JOB_ID \
  -H "Authorization: Bearer $TOKEN"

# 3. 模拟处理任务（需要后端实现处理逻辑）
# 这里假设有处理接口，实际可能需要手动触发或等待队列处理

# 4. 再次查询任务状态（应该是 completed）
curl https://api.factory-buy.com/jobs/$JOB_ID \
  -H "Authorization: Bearer $TOKEN"
```

**预期结果：**
- ✅ 任务创建成功
- ✅ 状态从 `queued` → `processing` → `completed`
- ✅ 输出包含 mock 数据
- ✅ 不产生真实费用

---

### 测试 6：Job 输入输出结构验证

**目标：** 验证 Job 输入输出符合 schema

**步骤：**

```bash
# 1. 创建有效任务（应该成功）
curl -X POST https://api.factory-buy.com/jobs \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "type": "text_generation",
    "input": {
      "prompt": "测试",
      "presetId": "qwen-turbo-standard"
    }
  }'

# 2. 创建无效任务（缺少必需字段，应该失败）
curl -X POST https://api.factory-buy.com/jobs \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "type": "text_generation",
    "input": {
      "presetId": "qwen-turbo-standard"
      // 缺少 prompt
    }
  }'
```

**预期结果：**
- ✅ 有效输入：成功创建任务
- ✅ 无效输入：返回 400 错误，提示验证失败

---

### 测试 7：性能索引验证

**目标：** 验证每日额度计算使用索引

**步骤：**

```bash
# 1. 查询今日消费（应该使用索引，不会全表扫描）
# 这个测试需要查看数据库执行计划，或者通过性能监控验证

# 2. 多次消费，验证性能
for i in {1..10}; do
  curl -X POST https://api.factory-buy.com/credits/consume \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $TOKEN" \
    -d "{
      \"amount\": 1,
      \"action\": \"perf_test_$i\"
    }"
done
```

**预期结果：**
- ✅ 每次消费响应时间 < 100ms
- ✅ 数据库查询使用索引（可通过 EXPLAIN 验证）

---

## 📊 测试结果记录

### 测试日期：___________

### 测试结果：

- [ ] 测试 1：幂等扣费 ✅ / ❌
- [ ] 测试 2：Dry Run 模型调用 ✅ / ❌
- [ ] 测试 3：Dry Run 扣费 ✅ / ❌
- [ ] 测试 4：参数白名单 ✅ / ❌
- [ ] 测试 5：Job 流程完整性 ✅ / ❌
- [ ] 测试 6：Job 结构验证 ✅ / ❌
- [ ] 测试 7：性能索引 ✅ / ❌

### 发现的问题：

1. _________________________________
2. _________________________________
3. _________________________________

---

## 🔧 快速测试脚本

```bash
#!/bin/bash

# 0 成本回归测试脚本

BACKEND_URL="https://api.factory-buy.com"
EMAIL="test@factory-buy.com"
PASSWORD="password123"

echo "🧪 开始 0 成本回归测试..."

# 1. 登录
echo "1️⃣ 登录..."
TOKEN=$(curl -s -X POST $BACKEND_URL/auth/login \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"$EMAIL\",\"password\":\"$PASSWORD\"}" \
  | jq -r '.token')

if [ "$TOKEN" == "null" ] || [ -z "$TOKEN" ]; then
  echo "❌ 登录失败"
  exit 1
fi
echo "✅ 登录成功"

# 2. 测试 Dry Run 模型调用
echo "2️⃣ 测试 Dry Run 模型调用..."
RESPONSE=$(curl -s -X POST $BACKEND_URL/ai/generate-text \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "prompt": "测试",
    "presetId": "qwen-turbo-standard"
  }')

if echo "$RESPONSE" | jq -e '.raw.mock == true' > /dev/null; then
  echo "✅ Dry Run 模式正常"
else
  echo "❌ Dry Run 模式异常"
  echo "$RESPONSE" | jq .
fi

# 3. 测试 Dry Run 扣费
echo "3️⃣ 测试 Dry Run 扣费..."
BALANCE_BEFORE=$(curl -s $BACKEND_URL/credits/balance \
  -H "Authorization: Bearer $TOKEN" | jq -r '.balance')

CONSUME_RESPONSE=$(curl -s -X POST $BACKEND_URL/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "amount": 10,
    "action": "test_dry_run"
  }')

if echo "$CONSUME_RESPONSE" | jq -e '.isDryRun == true' > /dev/null; then
  echo "✅ Dry Run 扣费正常"
else
  echo "❌ Dry Run 扣费异常"
  echo "$CONSUME_RESPONSE" | jq .
fi

BALANCE_AFTER=$(curl -s $BACKEND_URL/credits/balance \
  -H "Authorization: Bearer $TOKEN" | jq -r '.balance')

if [ "$BALANCE_BEFORE" == "$BALANCE_AFTER" ]; then
  echo "✅ 余额未变化（Dry Run 正常）"
else
  echo "❌ 余额发生变化（Dry Run 异常）"
fi

# 4. 测试幂等扣费
echo "4️⃣ 测试幂等扣费..."
REF_ID="test-idempotent-$(date +%s)"

FIRST=$(curl -s -X POST $BACKEND_URL/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d "{
    \"amount\": 5,
    \"action\": \"test_idempotent\",
    \"refId\": \"$REF_ID\"
  }")

SECOND=$(curl -s -X POST $BACKEND_URL/credits/consume \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d "{
    \"amount\": 5,
    \"action\": \"test_idempotent\",
    \"refId\": \"$REF_ID\"
  }")

if echo "$SECOND" | jq -e '.isIdempotent == true' > /dev/null; then
  echo "✅ 幂等扣费正常"
else
  echo "❌ 幂等扣费异常"
  echo "$SECOND" | jq .
fi

echo "🎉 测试完成！"
```

---

## 📝 注意事项

1. **Dry Run 模式必须配置**
   - `MODEL_ENABLED=false` 和 `BILLING_ENABLED=false` 必须同时设置
   - 否则可能产生真实费用

2. **生产环境验证**
   - 在生产环境测试时，确保 `NODE_ENV=production`
   - 验证参数白名单是否生效

3. **数据库约束**
   - 唯一约束 `(userId, refId, action)` 会在数据库层防止重复扣费
   - 即使代码层检查失败，数据库也会拒绝重复记录

---

**所有测试通过后，系统已具备验收条件！** ✅







