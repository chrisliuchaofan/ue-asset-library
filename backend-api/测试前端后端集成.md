# 🧪 前端与后端集成测试指南

## 📋 测试前准备

1. **确认后端服务运行正常**
   ```bash
   # 在服务器上执行
   pm2 status
   curl http://localhost:3001/health
   ```

2. **确认前端可以访问后端 API**
   - 后端 API 地址：`https://api.factory-buy.com`
   - 前端地址：`https://www.factory-buy.com`

3. **打开浏览器开发者工具**（F12）

---

## 1. 基础连接测试

### 1.1 测试后端健康检查

在浏览器控制台执行：

```javascript
// 测试健康检查
fetch('https://api.factory-buy.com/health')
  .then(r => r.json())
  .then(data => {
    console.log('✅ 后端健康检查成功:', data);
  })
  .catch(err => {
    console.error('❌ 后端健康检查失败:', err);
  });
```

**预期结果：**
```json
{
  "status": "ok",
  "timestamp": 1234567890
}
```

---

## 2. 用户认证测试

### 2.1 测试登录 API

在浏览器控制台执行：

```javascript
// 测试登录（需要先在后端配置用户）
fetch('https://api.factory-buy.com/auth/login', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    email: 'test@factory-buy.com',  // 替换为实际用户邮箱
    password: 'password123'         // 替换为实际密码
  })
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ 登录成功:', data);
    // 保存 token 供后续测试使用
    window.testToken = data.token;
  })
  .catch(err => {
    console.error('❌ 登录失败:', err);
  });
```

**预期结果：**
```json
{
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "user": {
    "email": "test@factory-buy.com"
  }
}
```

### 2.2 测试 Token 验证

```javascript
// 使用登录获得的 token
fetch('https://api.factory-buy.com/auth/verify', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    token: window.testToken  // 使用上面保存的 token
  })
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ Token 验证成功:', data);
  })
  .catch(err => {
    console.error('❌ Token 验证失败:', err);
  });
```

---

## 3. 积分系统测试

### 3.1 查询积分余额

```javascript
// 需要先登录获取 token
const userId = 'test@factory-buy.com';  // 替换为实际用户邮箱

fetch('https://api.factory-buy.com/credits/balance', {
  method: 'GET',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.testToken}`,  // 使用登录获得的 token
    'X-User-Id': userId
  }
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ 积分余额:', data);
  })
  .catch(err => {
    console.error('❌ 查询积分失败:', err);
  });
```

**预期结果：**
```json
{
  "balance": 100
}
```

### 3.2 消费积分

```javascript
const userId = 'test@factory-buy.com';

fetch('https://api.factory-buy.com/credits/consume', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.testToken}`,
    'X-User-Id': userId
  },
  body: JSON.stringify({
    amount: 10,
    action: 'test_action'
  })
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ 消费积分成功:', data);
  })
  .catch(err => {
    console.error('❌ 消费积分失败:', err);
  });
```

**预期结果：**
```json
{
  "success": true,
  "balance": 90,
  "transactionId": "txn-1234567890-abc123"
}
```

---

## 4. 日志系统测试

### 4.1 创建日志

```javascript
const userId = 'test@factory-buy.com';

fetch('https://api.factory-buy.com/logs/create', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.testToken}`,
    'X-User-Id': userId
  },
  body: JSON.stringify({
    action: 'test_action',
    details: {
      message: '测试日志',
      timestamp: new Date().toISOString()
    },
    success: true,
    timestamp: new Date().toISOString()
  })
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ 创建日志成功:', data);
  })
  .catch(err => {
    console.error('❌ 创建日志失败:', err);
  });
```

**预期结果：**
```json
{
  "logId": "uuid-12345678-1234-1234-1234-123456789abc"
}
```

---

## 5. 错误处理测试

### 5.1 测试未认证访问

```javascript
// 不提供 token
fetch('https://api.factory-buy.com/credits/balance', {
  method: 'GET',
  headers: {
    'Content-Type': 'application/json',
    'X-User-Id': 'test@factory-buy.com'
  }
})
  .then(r => r.json())
  .then(data => {
    console.log('响应:', data);
  })
  .catch(err => {
    console.error('❌ 预期错误（未认证）:', err);
  });
```

**预期结果：** 返回 401 未授权错误

### 5.2 测试积分不足

```javascript
// 尝试消费超过余额的积分
fetch('https://api.factory-buy.com/credits/consume', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.testToken}`,
    'X-User-Id': userId
  },
  body: JSON.stringify({
    amount: 999999,  // 超过余额
    action: 'test_action'
  })
})
  .then(r => r.json())
  .then(data => {
    console.log('响应:', data);
  })
  .catch(err => {
    console.error('❌ 预期错误（积分不足）:', err);
  });
```

**预期结果：** 返回 402 支付要求错误，包含 `INSUFFICIENT_CREDITS` 代码

---

## 6. CORS 测试

### 6.1 从前端页面测试

在 `https://www.factory-buy.com` 页面的控制台执行：

```javascript
// 测试 CORS 是否配置正确
fetch('https://api.factory-buy.com/health', {
  method: 'GET',
  credentials: 'include'  // 包含 cookies
})
  .then(r => r.json())
  .then(data => {
    console.log('✅ CORS 配置正确:', data);
  })
  .catch(err => {
    console.error('❌ CORS 配置错误:', err);
  });
```

**预期结果：** 成功返回数据，没有 CORS 错误

---

## 7. 完整流程测试

### 7.1 完整用户流程

```javascript
// 1. 登录
async function testFullFlow() {
  try {
    // 登录
    const loginRes = await fetch('https://api.factory-buy.com/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email: 'test@factory-buy.com',
        password: 'password123'
      })
    });
    const loginData = await loginRes.json();
    const token = loginData.token;
    const userId = 'test@factory-buy.com';
    
    console.log('✅ 1. 登录成功');
    
    // 2. 查询积分
    const balanceRes = await fetch('https://api.factory-buy.com/credits/balance', {
      headers: {
        'Authorization': `Bearer ${token}`,
        'X-User-Id': userId
      }
    });
    const balanceData = await balanceRes.json();
    console.log('✅ 2. 查询积分成功:', balanceData);
    
    // 3. 消费积分
    const consumeRes = await fetch('https://api.factory-buy.com/credits/consume', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
        'X-User-Id': userId
      },
      body: JSON.stringify({
        amount: 5,
        action: 'test_flow'
      })
    });
    const consumeData = await consumeRes.json();
    console.log('✅ 3. 消费积分成功:', consumeData);
    
    // 4. 创建日志
    const logRes = await fetch('https://api.factory-buy.com/logs/create', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
        'X-User-Id': userId
      },
      body: JSON.stringify({
        action: 'test_flow',
        details: { test: true },
        success: true,
        timestamp: new Date().toISOString()
      })
    });
    const logData = await logRes.json();
    console.log('✅ 4. 创建日志成功:', logData);
    
    console.log('🎉 完整流程测试通过！');
  } catch (error) {
    console.error('❌ 测试失败:', error);
  }
}

// 执行测试
testFullFlow();
```

---

## 8. 测试检查清单

- [ ] 后端健康检查正常
- [ ] 登录 API 正常工作
- [ ] Token 验证正常工作
- [ ] 查询积分余额正常
- [ ] 消费积分正常
- [ ] 创建日志正常
- [ ] 未认证访问正确返回 401
- [ ] 积分不足正确返回 402
- [ ] CORS 配置正确
- [ ] 完整流程测试通过

---

## 9. 常见问题排查

### 问题 1：CORS 错误

**症状：** 浏览器控制台显示 `CORS policy` 错误

**解决方案：**
1. 检查后端 `main.ts` 中的 CORS 配置
2. 确认 `FRONTEND_URL` 环境变量包含前端域名
3. 重启后端服务：`pm2 restart ue-assets-backend --update-env`

### 问题 2：401 未授权

**症状：** 所有需要认证的 API 都返回 401

**解决方案：**
1. 确认提供了正确的 `Authorization` header
2. 确认 token 未过期
3. 检查后端 `JWT_SECRET` 环境变量是否正确

### 问题 3：404 未找到

**症状：** API 返回 404

**解决方案：**
1. 确认 API 路径正确（注意大小写）
2. 检查 Nginx 配置是否正确代理到后端
3. 确认后端服务正在运行

### 问题 4：数据库连接错误

**症状：** 后端日志显示数据库连接失败

**解决方案：**
1. 检查 `.env` 文件中的数据库配置
2. 确认 PostgreSQL 服务正在运行
3. 检查数据库用户权限

---

## 10. 下一步

测试通过后，可以：

1. **集成到前端代码**
   - 创建前端 API 客户端封装
   - 在需要的地方调用后端 API
   - 处理错误和加载状态

2. **添加前端 UI**
   - 显示用户积分余额
   - 显示积分消费记录
   - 显示操作日志

3. **优化用户体验**
   - 添加加载动画
   - 添加错误提示
   - 添加成功提示

---

**测试完成后，请记录测试结果和发现的问题！**


