# M5.1 测试指南

## 📋 测试前准备

### 1. 检查环境变量

**后端 `.env` 文件需要包含：**
```bash
# OSS 配置（必需）
OSS_BUCKET=your-bucket-name
OSS_REGION=oss-cn-hangzhou
OSS_ACCESS_KEY=your-access-key-id
OSS_SECRET=your-access-key-secret

# OSS 访问 URL（可选，用于 CDN）
OSS_BASE_URL=https://your-cdn-domain.com

# 数据库配置
DB_HOST=localhost
DB_PORT=5432
DB_USERNAME=your-username
DB_PASSWORD=your-password
DB_NAME=ue_assets
```

**检查方法：**
```bash
cd backend-api
cat .env | grep OSS
```

---

### 2. 检查数据库表

**确认 `job_outputs` 表已创建：**

```sql
-- 连接到数据库
psql -h localhost -U your-username -d ue_assets

-- 检查表是否存在
\dt job_outputs

-- 查看表结构
\d job_outputs
```

**如果表不存在，TypeORM 会在启动时自动创建（如果 `synchronize: true`）。**

---

### 3. 检查临时目录

**确认 `/tmp/dream-factory` 目录存在：**

```bash
# 在服务器上执行
ls -la /tmp/dream-factory
```

**如果不存在，StorageService 会自动创建。**

---

## 🧪 测试步骤

### 测试 1：检查后端服务启动

**步骤：**
1. 启动后端服务
2. 检查日志，确认没有错误

```bash
cd backend-api
npm run start:dev
```

**预期结果：**
- ✅ 服务正常启动
- ✅ 没有 OSS 配置错误
- ✅ 数据库连接成功
- ✅ `job_outputs` 表已创建

---

### 测试 2：测试 OSS 配置

**创建测试脚本：** `backend-api/test-oss-config.ts`

```typescript
import { StorageService } from './src/storage/storage.service';
import { Test } from '@nestjs/testing';
import { getRepositoryToken } from '@nestjs/typeorm';
import { JobOutput } from './src/database/entities/job-output.entity';

async function testOSSConfig() {
  const mockRepository = {
    create: jest.fn(),
    save: jest.fn(),
    find: jest.fn(),
  };

  const module = await Test.createTestingModule({
    providers: [
      StorageService,
      {
        provide: getRepositoryToken(JobOutput),
        useValue: mockRepository,
      },
    ],
  }).compile();

  const storageService = module.get<StorageService>(StorageService);
  
  // 测试 OSS 客户端创建
  try {
    const client = (storageService as any).getOSSClient();
    console.log('✅ OSS 客户端创建成功');
  } catch (error) {
    console.error('❌ OSS 配置错误:', error);
  }
}

testOSSConfig();
```

**或者直接检查环境变量：**
```bash
cd backend-api
node -e "console.log('OSS_BUCKET:', process.env.OSS_BUCKET); console.log('OSS_REGION:', process.env.OSS_REGION);"
```

---

### 测试 3：测试图片生成和上传

**使用 Postman 或 curl：**

```bash
# 1. 先登录获取 token
TOKEN=$(curl -X POST http://localhost:3001/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"your-password"}' \
  | jq -r '.token')

# 2. 调用生成图片接口（提供已生成的图片 URL）
curl -X POST http://localhost:3001/ai/generate-image \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "测试图片",
    "imageUrl": "https://via.placeholder.com/1024x1024.jpg"
  }'
```

**预期结果：**
- ✅ 返回 `imageUrl`（OSS URL）
- ✅ OSS 中有文件（路径：`users/{userId}/jobs/{jobId}/image-{timestamp}.jpg`）
- ✅ 数据库 `job_outputs` 表有记录
- ✅ `/tmp/dream-factory/` 目录没有残留文件

---

### 测试 4：测试视频生成和上传

```bash
# 调用生成视频接口（提供已生成的视频 URL）
curl -X POST http://localhost:3001/ai/generate-video \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "video",
    "imageUrl": "https://via.placeholder.com/1920x1080.jpg",
    "prompt": "测试视频",
    "videoUrl": "https://sample-videos.com/video123/mp4/720/big_buck_bunny_720p_1mb.mp4"
  }'
```

**预期结果：**
- ✅ 返回 `videoUrl`（OSS URL）
- ✅ OSS 中有文件（路径：`users/{userId}/jobs/{jobId}/video-{timestamp}.mp4`）
- ✅ 数据库 `job_outputs` 表有记录
- ✅ `/tmp/dream-factory/` 目录没有残留文件

---

### 测试 5：检查 OSS 文件

**方法 1：使用 OSS 控制台**
1. 登录阿里云 OSS 控制台
2. 进入对应的 Bucket
3. 查看路径：`users/{userId}/jobs/{jobId}/`
4. 确认有文件存在

**方法 2：使用 OSS CLI**
```bash
# 列出文件
ossutil ls oss://your-bucket/users/test@factory-buy.com/jobs/
```

---

### 测试 6：检查数据库记录

```sql
-- 查询 job_outputs 表
SELECT 
  id,
  "jobId",
  "userId",
  type,
  "ossUrl",
  "ossPath",
  size,
  meta,
  "createdAt"
FROM job_outputs
ORDER BY "createdAt" DESC
LIMIT 10;
```

**预期结果：**
- ✅ 有记录
- ✅ `ossUrl` 是完整的 OSS URL
- ✅ `ossPath` 符合规范：`users/{userId}/jobs/{jobId}/{type}-{timestamp}.{ext}`
- ✅ `size` 大于 0

---

### 测试 7：检查临时文件清理

**在服务器上执行：**
```bash
# 检查临时目录
ls -la /tmp/dream-factory/

# 应该只有正在处理的文件，没有已完成的文件
```

**预期结果：**
- ✅ 没有已完成的 Job 的临时文件
- ✅ 只有正在处理的文件（如果有）

---

### 测试 8：测试清理机制

**手动触发清理：**
```bash
# 在服务器上执行（需要 Node.js 环境）
cd backend-api
node -e "
const { StorageService } = require('./dist/storage/storage.service');
const service = new StorageService();
service.cleanupOldTempFiles().then(count => {
  console.log('清理了', count, '个过期文件');
});
"
```

**预期结果：**
- ✅ 清理 24 小时前的临时文件
- ✅ 返回清理的文件数量

---

## ⚠️ 常见问题

### 问题 1：OSS 配置错误

**错误信息：**
```
OSS 配置不完整，请检查环境变量：OSS_BUCKET, OSS_REGION, OSS_ACCESS_KEY, OSS_SECRET
```

**解决方法：**
1. 检查 `.env` 文件中的 OSS 配置
2. 确保所有必需的环境变量都已设置
3. 重启后端服务

---

### 问题 2：OSS 上传失败

**错误信息：**
```
上传图片到 OSS 失败: AccessDenied
```

**解决方法：**
1. 检查 OSS AccessKey 和 Secret 是否正确
2. 检查 OSS Bucket 权限设置
3. 确保有写入权限

---

### 问题 3：临时文件未清理

**检查：**
```bash
ls -la /tmp/dream-factory/
```

**解决方法：**
1. 检查 Job 是否正常完成（`status = 'completed'`）
2. 检查 `JobsService.complete()` 是否被调用
3. 手动清理：`rm -rf /tmp/dream-factory/*`

---

### 问题 4：数据库表不存在

**错误信息：**
```
relation "job_outputs" does not exist
```

**解决方法：**
1. 检查 `synchronize: true`（开发环境）
2. 或运行数据库迁移
3. 手动创建表（参考 `job-output.entity.ts`）

---

## 📊 测试检查清单

- [ ] 后端服务正常启动
- [ ] OSS 配置正确
- [ ] 数据库 `job_outputs` 表存在
- [ ] 图片生成和上传成功
- [ ] 视频生成和上传成功
- [ ] OSS 中有文件
- [ ] 数据库有记录
- [ ] 临时文件已清理
- [ ] 返回的 URL 是 OSS URL（不是本地路径）

---

## 🎯 下一步

测试通过后：
1. 修改前端 API 路由，生成后调用后端上传
2. 实现 Cron 清理任务（可选）
3. 更新环境变量配置文档







