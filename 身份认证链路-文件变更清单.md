# 📋 身份认证链路 - 文件变更清单

## ✅ 新增文件

### 1. `lib/backend-api-client.ts`
**功能：** 后端 API 客户端，自动处理认证 token 获取和携带

**关键方法：**
- `getBackendToken()` - 获取后端 JWT token（使用 session email + 配置的密码）
- `callBackendAPI()` - 调用后端 API（自动携带认证）
- `getCurrentUserInfo()` - 获取当前用户信息（调用 `/me` 接口）

### 2. `身份认证链路实现总结.md`
**功能：** 详细的实现文档和验证步骤

### 3. `环境变量配置清单.md`
**功能：** 完整的环境变量配置指南

### 4. `身份认证链路-文件变更清单.md`（本文件）
**功能：** 文件变更清单

---

## 📝 修改文件

### 前端文件

#### 1. `middleware.ts`
**变更：**
- ✅ 增加 `/dream-factory` 路由保护
- ✅ 未登录自动跳转到 `/auth/login`

**关键代码：**
```typescript
// 保护 /admin 和 /dream-factory 路由
if (pathname.startsWith('/admin') || pathname.startsWith('/dream-factory')) {
  const isAuthenticated = !!req.auth;
  if (!isAuthenticated) {
    const loginUrl = new URL('/auth/login', req.url);
    loginUrl.searchParams.set('callbackUrl', pathname);
    return NextResponse.redirect(loginUrl);
  }
}
```

#### 2. `app/api/ai/generate-text/route.ts`
**变更：**
- ✅ 增加登录状态检查（未登录返回 401）
- ✅ 使用 `callBackendAPI` 调用后端（自动携带认证）
- ✅ 移除旧的 `getBackendToken` 函数

**关键代码：**
```typescript
// ✅ 检查登录状态
const session = await getSession();
if (!session?.user?.email) {
  return NextResponse.json(
    { message: '未登录，请先登录' },
    { status: 401 }
  );
}

// 调用后端API（自动携带认证）
const backendResult = await callBackendAPI('/ai/generate-text', {
  method: 'POST',
  body: JSON.stringify(backendRequestBody),
});
```

#### 3. `app/api/ai/generate-image/route.ts`
**变更：**
- ✅ 增加登录状态检查（未登录返回 401）

**关键代码：**
```typescript
const session = await getSession();
if (!session?.user?.email) {
  return NextResponse.json(
    { message: '未登录，请先登录' },
    { status: 401 }
  );
}
```

#### 4. `app/api/ai/generate-job/route.ts`
**变更：**
- ✅ 增加登录状态检查（未登录返回 401）

#### 5. `app/dream-factory/page.tsx`
**变更：**
- ✅ 增加用户信息显示（右上角）
- ✅ 调用 `/me` 接口获取用户信息
- ✅ 显示余额和模式（DRY_RUN/REAL）
- ✅ 退出登录按钮

**关键代码：**
```typescript
// 用户信息状态
const [userInfo, setUserInfo] = useState<{
  userId: string;
  email: string;
  balance: number;
  billingMode: 'DRY_RUN' | 'REAL';
  modelMode: 'DRY_RUN' | 'REAL';
} | null>(null);

// 加载用户信息
useEffect(() => {
  if (session?.user?.email) {
    getCurrentUserInfo()
      .then(setUserInfo)
      .catch((err) => {
        console.error('[Dream Factory] 获取用户信息失败:', err);
      });
  }
}, [session]);
```

#### 6. `lib/auth-config.ts`
**变更：**
- ✅ 支持 email 格式的用户名
- ✅ 自动生成 email（如果 username 不包含 @）

**关键代码：**
```typescript
function getAdminUsers(): Array<{ username: string; password: string; email?: string }> {
  // ...
  return usersEnv.split(',').map((user) => {
    const [username, password] = user.split(':');
    const usernameTrimmed = username.trim();
    const email = usernameTrimmed.includes('@') 
      ? usernameTrimmed 
      : `${usernameTrimmed}@admin.local`;
    return { 
      username: usernameTrimmed, 
      password: password.trim(),
      email,
    };
  });
}
```

### 后端文件

#### 7. `backend-api/src/auth/auth.controller.ts`
**变更：**
- ✅ 新增 `MeController` 类
- ✅ 新增 `/me` 接口
- ✅ 返回用户信息、余额、模式信息

**关键代码：**
```typescript
@Controller()
@UseGuards(AuthGuard)
export class MeController {
  @Get('me')
  async getMe(@CurrentUser() user: { userId: string; email: string }) {
    const balanceResult = await this.creditsService.getBalance(user.userId);
    
    // 判断模式（根据环境变量）
    const modelEnabled = process.env.MODEL_ENABLED !== 'false';
    const billingEnabled = process.env.BILLING_ENABLED !== 'false';
    
    // 可以根据用户白名单覆盖模式
    const realModeUsers = process.env.REAL_MODE_USERS || '';
    let finalModelMode: 'DRY_RUN' | 'REAL' = modelEnabled ? 'REAL' : 'DRY_RUN';
    let finalBillingMode: 'DRY_RUN' | 'REAL' = billingEnabled ? 'REAL' : 'DRY_RUN';
    
    if (realModeUsers && realModeUsers.split(',').includes(user.email)) {
      finalModelMode = 'REAL';
      finalBillingMode = 'REAL';
    }
    
    return {
      userId: user.userId,
      email: user.email,
      balance: balanceResult.balance,
      billingMode: finalBillingMode,
      modelMode: finalModelMode,
    };
  }
}
```

#### 8. `backend-api/src/auth/auth.module.ts`
**变更：**
- ✅ 导入 `CreditsModule`
- ✅ 注册 `MeController`

**关键代码：**
```typescript
@Module({
  imports: [CreditsModule],
  controllers: [AuthController, MeController],
  providers: [AuthService],
  exports: [AuthService],
})
export class AuthModule {}
```

---

## 🔧 环境变量变更

### 前端新增环境变量

```env
# 后端登录密码（用于获取后端 token）
BACKEND_TEST_PASSWORD=test123
```

### 后端新增环境变量

```env
# 真实模式用户白名单（可选）
REAL_MODE_USERS=admin@factory-buy.com
```

---

## 📊 变更统计

- **新增文件：** 4 个
- **修改文件：** 8 个
- **新增接口：** 1 个（`/me`）
- **保护路由：** 1 个（`/dream-factory`）
- **保护 API：** 3 个（`/api/ai/generate-text`, `/api/ai/generate-image`, `/api/ai/generate-job`）

---

## ✅ 验证清单

### 功能验证

- [x] `/dream-factory` 路由保护（未登录跳转登录页）
- [x] 登录后自动跳转回原页面
- [x] 前端生成接口需要登录（401 保护）
- [x] 后端 `/me` 接口返回用户信息
- [x] 前端显示用户信息（右上角）
- [x] 前端显示余额和模式
- [x] 后端自动识别 userId（从 token 解析）
- [x] Dry Run 模式正常工作

### 配置验证

- [x] 前端 `NEXTAUTH_SECRET` 与后端 `JWT_SECRET` 相同
- [x] 前端 `ADMIN_USERS` 密码与后端 `USER_WHITELIST` 匹配
- [x] 后端 `MODEL_ENABLED=false` 和 `BILLING_ENABLED=false` 配置正确

---

## 🚀 部署步骤

### 1. 前端部署（Vercel）

1. 在 Vercel 环境变量中添加：
   - `NEXTAUTH_SECRET`
   - `ADMIN_USERS`
   - `BACKEND_TEST_PASSWORD`
   - `NEXT_PUBLIC_BACKEND_API_URL`

2. 重新部署

### 2. 后端部署（服务器）

1. 在服务器 `.env` 文件中添加：
   - `JWT_SECRET`（与前端相同）
   - `USER_WHITELIST`（密码与前端匹配）
   - `MODEL_ENABLED=false`
   - `BILLING_ENABLED=false`

2. 重启服务：
   ```bash
   pm2 restart ue-assets-backend --update-env
   ```

---

## 📝 测试步骤

### 1. 测试路由保护

```bash
# 访问 /dream-factory（未登录）
curl -I http://localhost:3000/dream-factory
# 应该返回 302 重定向到 /auth/login
```

### 2. 测试 API 认证

```bash
# 未登录调用生成接口
curl -X POST http://localhost:3000/api/ai/generate-text \
  -H "Content-Type: application/json" \
  -d '{"prompt":"测试"}'
# 应该返回 401
```

### 3. 测试 /me 接口

```bash
# 1. 登录获取 token
TOKEN=$(curl -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"test@factory-buy.com","password":"test123"}' \
  | jq -r '.token')

# 2. 调用 /me 接口
curl https://api.factory-buy.com/me \
  -H "Authorization: Bearer $TOKEN" | jq .
```

---

## ✅ 完成状态

所有功能已实现并测试通过！🎉

