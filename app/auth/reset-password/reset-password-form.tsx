...
function getCompanyEmailHint(allowedDomains: string[]) {
  ...
  return `仅支持 ${allowedDomains.map((domain) => `@${domain}`).join('、')} 公司邮箱`;
}
...
export function ResetPasswordForm({ allowedDomains }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.message || '重设密码失败');
        return;
      }

      setSuccess(data.message || '密码已重设，请使用新密码登录');
      setTimeout(() => {
        router.push('/auth/login');
      }, 1600);
    } catch (err: any) {
      setError(`重设密码失败: ${err.message || '请重试'}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold">重设密码</CardTitle>
