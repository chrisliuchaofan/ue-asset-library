...
async function callAIImageAPI(
  ...
  const buildRequestBody = (): any => {
    ...
    // 如果 skipTags 为 true（资产详情页场景），只要求描述，不要求标签，且限制50字以内
    const defaultOpenAIPrompt = skipTags 
      ? '请详细分析这张图片的内容，包括风格、主题、元素、色彩、构图等方面，提供简洁的中文描述。描述总字数不超过50字。仅输出纯文本描述，不需要标签，不要使用代码格式。'
      : '请分析这张图片，提供标签（用逗号分隔）和简短描述。格式：标签：xxx,xxx,xxx\n描述：xxx';
    
    const defaultAliyunPrompt = skipTags
      ? '你是资深游戏美术分析师，请详细分析这张图片的内容，包括风格、主题、元素、色彩、构图等方面，提供简洁的中文描述。描述总字数不超过50字。仅输出 JSON 格式：{"description":""}，不需要 tags 字段。描述应为纯文本，不要包含代码、标记或特殊格式。'
