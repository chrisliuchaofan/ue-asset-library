...
**问题**: 数据过滤逻辑每次渲染都重新计算，浪费 CPU 资源

**解决方案**:
- 使用 `useMemo` 缓存过滤结果，减少重复计算
- 使用 `useDeferredValue` 延迟非关键更新，提升交互响应
- 优化 `hasServerFilters` 计算，使用 `useMemo` 缓存

**文件**: 
- `components/assets-list-with-selection.tsx`
...
**问题**: 首屏图片尺寸仍然较大，影响 LCP

**解决方案**:
- 进一步减小首屏图片尺寸（从 240-320px 降到 200-280px）
- 优化非首屏图片尺寸（从 300-400px 降到 280-360px）
- 优化 `sizes` 属性，使用动态计算的图片宽度
...
**问题**: CartDialog 等非关键组件在首屏加载

**解决方案**:
- CartDialog 使用动态导入（`lazy`），只在需要时加载
- 减少首屏 JavaScript 包大小
...
**预期效果**: 
- 减少初始 JavaScript 包大小约 20-30 KiB
- 提升首屏加载速度
