...
2. 后端 token 获取逻辑：
   - 使用 `session.user.email` 作为后端登录的 email
   - 从 `ADMIN_USERS` 环境变量中查找匹配的密码
   - 或使用 `BACKEND_TEST_PASSWORD`（如果配置）
3. **问题：** 如果前端 `session.user.email` 与后端 `USER_WHITELIST` 不匹配，**后端登录会失败**
...
1. **前后端用户身份链路断裂风险**
   - **问题：** 前端 `session.user.email` 与后端 `USER_WHITELIST` 不匹配时，后端登录会失败
   - **影响：** 用户无法使用需要后端认证的功能（积分查询、AI 生成等）
   - **严重性：** 🔴 高

2. **前端 AI 调用未完全接入后端计费系统**
   - **问题：** AI 图像分析（`/api/ai/analyze-image`）直接调用真实 AI API，不检查 Dry Run 模式，不调用后端计费系统
   - **影响：** 可能产生真实费用，但不会扣用户积分，用户不知道实际花费
   - **严重性：** 🔴 高

3. **Dry Run 模式不一致**
   - **问题：** 前端 AI 图像分析没有 Dry Run flag，依赖环境变量配置；后端有 Dry Run flag，但前端可能绕过后端
