# 🚀 部署指南 - 小白版（一步一步来）

> 本指南专为新手设计，每一步都有详细说明和截图提示

## 📋 部署前准备清单

在开始之前，请确认你已经准备好：

- [ ] ✅ **域名**（你已经准备好了）
- [ ] ⬜ **Vercel 账号**（如果没有，需要注册：https://vercel.com）
- [ ] ⬜ **GitHub/GitLab 账号**（用于存放代码）
- [ ] ⬜ **阿里云 OSS 配置**（如果使用云存储）
- [ ] ⬜ **后端服务器**（如果有后端 API）

---

## 第一步：准备代码仓库

### 1.1 将代码推送到 GitHub

如果你还没有把代码推送到 GitHub，按以下步骤操作：

1. **在 GitHub 创建新仓库**
   - 访问 https://github.com/new
   - 仓库名称：`ue-asset-library`（或你喜欢的名字）
   - 选择 **Public** 或 **Private**
   - 点击 **Create repository**

2. **在本地推送代码**

打开终端（Terminal），进入项目目录：

```bash
# 进入项目目录
cd /Users/chrisl/Documents/恒星UE资产库/web

# 初始化 Git（如果还没有）
git init

# 添加所有文件
git add .

# 提交代码
git commit -m "Initial commit"

# 添加远程仓库（替换 YOUR_USERNAME 和 YOUR_REPO_NAME）
git remote add origin https://github.com/YOUR_USERNAME/YOUR_REPO_NAME.git

# 推送代码
git branch -M main
git push -u origin main
```

> 💡 **提示**：如果遇到权限问题，可能需要配置 SSH 密钥或使用 GitHub Desktop

---

## 第二步：部署到 Vercel（前端）

### 2.1 登录 Vercel

1. 访问 https://vercel.com
2. 点击 **Sign Up** 注册账号（可以用 GitHub 账号登录）
3. 登录后进入 Dashboard

### 2.2 导入项目

1. 在 Vercel Dashboard 点击 **Add New** → **Project**
2. 选择 **Import Git Repository**
3. 找到你的仓库，点击 **Import**

### 2.3 配置项目

Vercel 会自动检测到这是 Next.js 项目，通常不需要修改，但请确认：

- **Framework Preset**: `Next.js` ✅
- **Root Directory**: `./` （如果项目在根目录）
- **Build Command**: `npm run build` ✅
- **Output Directory**: `.next` ✅

> ⚠️ **注意**：如果你的项目在 `web` 子目录，需要：
> - 点击 **Root Directory** 旁边的 **Edit**
> - 输入 `web`
> - 点击 **Save**

### 2.4 配置环境变量（重要！）

在部署之前，需要配置环境变量。点击 **Environment Variables** 添加以下变量：

#### 🔐 必须配置的变量：

```env
# NextAuth 认证（必须）
NEXTAUTH_SECRET=你的密钥（下面会教你怎么生成）
NEXTAUTH_URL=https://你的域名.vercel.app

# 后端 API（如果有后端）
BACKEND_API_URL=https://api.你的域名.com

# 用户白名单（测试账号）
USER_WHITELIST=test@example.com:password123
```

#### 📦 OSS 配置（如果使用阿里云 OSS）：

```env
OSS_BUCKET=你的bucket名称
OSS_REGION=oss-cn-hangzhou
OSS_ACCESS_KEY_ID=你的AccessKey
OSS_ACCESS_KEY_SECRET=你的SecretKey
NEXT_PUBLIC_CDN_BASE=https://你的CDN域名.com
```

#### 🤖 AI 配置（如果使用 AI 功能）：

```env
AI_IMAGE_API_KEY=你的AI密钥
AI_IMAGE_API_ENDPOINT=https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation
```

#### 🎬 即梦视频配置（如果使用）：

```env
JIMENG_REQ_KEY=jimeng_i2v_first_v30
```

### 2.5 生成密钥

在配置 `NEXTAUTH_SECRET` 之前，需要生成一个随机密钥：

**方法 1：使用在线工具**
- 访问 https://generate-secret.vercel.app/32
- 复制生成的密钥

**方法 2：使用命令行**
```bash
# Mac/Linux
openssl rand -base64 32

# Windows (PowerShell)
[Convert]::ToBase64String((1..32 | ForEach-Object { Get-Random -Maximum 256 }))
```

### 2.6 开始部署

1. 确认所有环境变量都已添加
2. 点击 **Deploy** 按钮
3. 等待部署完成（通常 2-5 分钟）

> 💡 **提示**：部署过程中可以在 **Deployments** 标签页查看日志

---

## 第三步：配置域名

### 3.1 在 Vercel 添加域名

1. 进入项目 → **Settings** → **Domains**
2. 在输入框中输入你的域名（例如：`example.com`）
3. 点击 **Add**
4. Vercel 会显示需要配置的 DNS 记录

### 3.2 配置 DNS 记录

根据 Vercel 的提示，在你的域名服务商（如阿里云、腾讯云）添加 DNS 记录：

**通常需要添加：**

| 类型 | 名称 | 值 | TTL |
|------|------|-----|-----|
| A | @ | 76.76.21.21 | 自动 |
| CNAME | www | cname.vercel-dns.com | 自动 |

> 💡 **提示**：Vercel 会显示具体的 DNS 配置，按照提示操作即可

### 3.3 等待 DNS 生效

DNS 生效通常需要几分钟到几小时，你可以：

1. 在 Vercel 的 Domains 页面查看状态
2. 当显示 **Valid Configuration** 时，域名配置成功

---

## 第四步：测试部署

### 4.1 访问网站

1. 打开浏览器，访问你的域名
2. 如果看到网站正常显示，说明部署成功！🎉

### 4.2 测试功能

- [ ] 首页能正常打开
- [ ] 可以搜索资产
- [ ] 可以查看资产详情
- [ ] 登录功能正常（如果配置了）

### 4.3 检查控制台

如果遇到问题：

1. 打开浏览器开发者工具（F12）
2. 查看 **Console** 标签页是否有错误
3. 查看 **Network** 标签页，检查请求是否成功

---

## 第五步：部署后端（如果有）

如果你的项目有后端 API，需要部署到服务器：

### 5.1 准备服务器

你需要一台云服务器（如阿里云 ECS、腾讯云 CVM）：

1. 购买服务器（建议 2核4G 起步）
2. 选择 Ubuntu 或 CentOS 系统
3. 记录服务器的公网 IP

### 5.2 连接服务器

使用 SSH 连接服务器：

```bash
ssh root@你的服务器IP
```

> 💡 **提示**：Windows 用户可以使用 PuTTY 或 Windows Terminal

### 5.3 安装 Node.js

```bash
# 更新系统
sudo apt update && sudo apt upgrade -y

# 安装 Node.js 20
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt-get install -y nodejs

# 验证安装
node --version
npm --version
```

### 5.4 上传后端代码

**方法 1：使用 Git**

```bash
# 在服务器上
cd /opt
git clone https://github.com/YOUR_USERNAME/YOUR_REPO_NAME.git
cd YOUR_REPO_NAME/backend-api
npm install
npm run build
```

**方法 2：使用 SCP（从本地）**

```bash
# 在本地终端执行
scp -r backend-api/* root@你的服务器IP:/opt/ue-assets-backend/
```

### 5.5 配置环境变量

在服务器上创建 `.env` 文件：

```bash
cd /opt/ue-assets-backend
nano .env
```

添加以下内容：

```env
PORT=3001
NODE_ENV=production
FRONTEND_URL=https://你的域名.com
JWT_SECRET=你的密钥（与Vercel的NEXTAUTH_SECRET相同）
USER_WHITELIST=test@example.com:password123
INITIAL_CREDITS=100
```

保存：按 `Ctrl + X`，然后按 `Y`，最后按 `Enter`

### 5.6 启动服务

```bash
# 安装 PM2（进程管理器）
sudo npm install -g pm2

# 启动服务
pm2 start dist/main.js --name ue-assets-backend

# 设置开机自启
pm2 startup
pm2 save

# 查看状态
pm2 status
```

### 5.7 配置 Nginx（可选，推荐）

```bash
# 安装 Nginx
sudo apt install nginx

# 创建配置文件
sudo nano /etc/nginx/sites-available/ue-assets-backend
```

添加以下内容：

```nginx
server {
    listen 80;
    server_name api.你的域名.com;

    location / {
        proxy_pass http://localhost:3001;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
    }
}
```

启用配置：

```bash
sudo ln -s /etc/nginx/sites-available/ue-assets-backend /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx
```

### 5.8 配置 SSL（HTTPS）

```bash
# 安装 Certbot
sudo apt install certbot python3-certbot-nginx

# 申请证书
sudo certbot --nginx -d api.你的域名.com

# 自动续期测试
sudo certbot renew --dry-run
```

### 5.9 配置后端域名 DNS

在你的域名服务商添加 A 记录：

| 类型 | 名称 | 值 | TTL |
|------|------|-----|-----|
| A | api | 你的服务器IP | 600 |

---

## 第六步：更新 Vercel 环境变量

部署后端后，需要更新 Vercel 的环境变量：

1. 进入 Vercel 项目 → **Settings** → **Environment Variables**
2. 找到 `BACKEND_API_URL`
3. 更新为：`https://api.你的域名.com`
4. 重新部署（Vercel 会自动触发，或手动点击 **Redeploy**）

---

## 🎉 完成！

恭喜！你的网站已经部署成功了！

### 常用操作

**查看 Vercel 部署日志：**
- 进入项目 → **Deployments** → 点击某个部署 → 查看日志

**重新部署：**
- 在 Vercel Dashboard 点击 **Redeploy**

**更新代码：**
```bash
# 修改代码后
git add .
git commit -m "更新说明"
git push
# Vercel 会自动重新部署
```

**查看后端日志：**
```bash
ssh root@你的服务器IP
pm2 logs ue-assets-backend
```

---

## ❓ 常见问题

### Q1: 部署失败怎么办？

**检查清单：**
- [ ] 环境变量是否都配置了？
- [ ] `NEXTAUTH_SECRET` 是否设置了？
- [ ] 代码是否有语法错误？
- [ ] 查看 Vercel 部署日志中的错误信息

### Q2: 域名无法访问？

**检查清单：**
- [ ] DNS 记录是否正确配置？
- [ ] 等待 DNS 生效（可能需要几小时）
- [ ] 在 Vercel Domains 页面检查状态

### Q3: 后端 API 无法连接？

**检查清单：**
- [ ] 服务器防火墙是否开放了 3001 端口？
- [ ] 后端服务是否正常运行？(`pm2 status`)
- [ ] `BACKEND_API_URL` 是否正确？
- [ ] CORS 配置是否正确？

### Q4: 如何查看错误日志？

**Vercel：**
- 项目 → Deployments → 点击部署 → 查看 Build Logs

**后端：**
```bash
pm2 logs ue-assets-backend
```

---

## 📞 需要帮助？

如果遇到问题，可以：
1. 查看 Vercel 官方文档：https://vercel.com/docs
2. 查看项目中的其他文档（如 `DEPLOYMENT_GUIDE.md`）
3. 检查错误日志，根据错误信息搜索解决方案

祝部署顺利！🎊


