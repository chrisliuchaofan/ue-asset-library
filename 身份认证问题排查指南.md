# 🔍 身份认证问题排查指南

## 问题 1: 后端登录失败 401 Unauthorized

### 原因分析

前端登录时使用的 email 和后端 `USER_WHITELIST` 中的 email 格式不匹配。

**前端配置：**
```env
ADMIN_USERS=admin:admin123
BACKEND_TEST_PASSWORD=password123
```

**前端登录流程：**
1. 用户使用 `admin:admin123` 登录
2. NextAuth 生成 session，email 为 `admin@admin.local`
3. 前端调用后端登录时，使用 `admin@admin.local` + `admin123`（从 ADMIN_USERS 匹配）

**后端配置要求：**
```env
USER_WHITELIST=admin@admin.local:admin123
# 或者
USER_WHITELIST=admin:admin123
```

### 解决方案

#### 方案 1: 修改后端 USER_WHITELIST（推荐）

在后端 `.env` 文件中配置：

```env
# 使用完整的 email 格式
USER_WHITELIST=admin@admin.local:admin123

# 或者使用用户名格式（后端已支持匹配）
USER_WHITELIST=admin:admin123
```

#### 方案 2: 修改前端 ADMIN_USERS

在前端 `.env.local` 文件中使用 email 格式：

```env
ADMIN_USERS=admin@factory-buy.com:admin123
```

然后后端配置：

```env
USER_WHITELIST=admin@factory-buy.com:admin123
```

### 验证步骤

1. **检查前端 session email：**
   ```javascript
   // 在浏览器控制台
   fetch('/api/auth/session').then(r => r.json()).then(console.log)
   ```

2. **检查后端 USER_WHITELIST：**
   ```bash
   # 在服务器上
   grep USER_WHITELIST /opt/ue-assets-backend/backend-api/.env
   ```

3. **测试后端登录：**
   ```bash
   curl -X POST https://api.factory-buy.com/auth/login \
     -H "Content-Type: application/json" \
     -d '{"email":"admin@admin.local","password":"admin123"}'
   ```

---

## 问题 2: 后端 /me 接口 404 Not Found

### 原因分析

后端 `/me` 接口可能：
1. 没有正确注册到路由
2. 后端服务未启动
3. 路由路径配置错误

### 解决方案

#### 1. 检查后端服务是否运行

```bash
# 在服务器上
pm2 list
pm2 logs ue-assets-backend --lines 50
```

#### 2. 检查路由注册

确保 `backend-api/src/auth/auth.module.ts` 中注册了 `MeController`：

```typescript
@Module({
  imports: [CreditsModule],
  controllers: [AuthController, MeController], // ✅ 确保包含 MeController
  providers: [AuthService],
  exports: [AuthService],
})
export class AuthModule {}
```

#### 3. 重启后端服务

```bash
# 在服务器上
cd /opt/ue-assets-backend/backend-api
pm2 restart ue-assets-backend --update-env
```

#### 4. 验证路由

```bash
# 先登录获取 token
TOKEN=$(curl -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@admin.local","password":"admin123"}' \
  | jq -r '.token')

# 测试 /me 接口
curl https://api.factory-buy.com/me \
  -H "Authorization: Bearer $TOKEN"
```

---

## 🔧 快速修复脚本

### 修复后端配置

```bash
# 在服务器上执行
cat >> /opt/ue-assets-backend/backend-api/.env << 'EOF'

# 身份认证配置（与前端匹配）
USER_WHITELIST=admin@admin.local:admin123
JWT_SECRET=你的密钥（与前端 NEXTAUTH_SECRET 相同）
NEXTAUTH_SECRET=你的密钥（与前端相同）
MODEL_ENABLED=false
BILLING_ENABLED=false
INITIAL_CREDITS=100
EOF

# 重启服务
pm2 restart ue-assets-backend --update-env
```

### 修复前端配置

```bash
# 在项目根目录执行
cat >> .env.local << 'EOF'
# 确保密码与后端匹配
ADMIN_USERS=admin:admin123
BACKEND_TEST_PASSWORD=admin123
NEXT_PUBLIC_BACKEND_API_URL=https://api.factory-buy.com
EOF
```

---

## ✅ 配置检查清单

### 前端配置

- [ ] `ADMIN_USERS` 已配置
- [ ] `BACKEND_TEST_PASSWORD` 已配置（或 ADMIN_USERS 中配置了密码）
- [ ] `NEXT_PUBLIC_BACKEND_API_URL` 已配置
- [ ] `NEXTAUTH_SECRET` 已配置

### 后端配置

- [ ] `USER_WHITELIST` 已配置（格式：`email:password` 或 `username:password`）
- [ ] `JWT_SECRET` 已配置（与前端 `NEXTAUTH_SECRET` 相同）
- [ ] `NEXTAUTH_SECRET` 已配置（与前端相同）
- [ ] `MODEL_ENABLED=false` 已配置
- [ ] `BILLING_ENABLED=false` 已配置
- [ ] `INITIAL_CREDITS` 已配置

### 密码匹配检查

**前端 `ADMIN_USERS` 和后端 `USER_WHITELIST` 的密码必须匹配：**

**示例 1（推荐）：**
```env
# 前端
ADMIN_USERS=admin:admin123

# 后端
USER_WHITELIST=admin@admin.local:admin123
# 或
USER_WHITELIST=admin:admin123
```

**示例 2：**
```env
# 前端
ADMIN_USERS=admin@factory-buy.com:admin123

# 后端
USER_WHITELIST=admin@factory-buy.com:admin123
```

---

## 🐛 调试技巧

### 1. 查看前端 session

在浏览器控制台：
```javascript
fetch('/api/auth/session').then(r => r.json()).then(console.log)
```

### 2. 查看后端日志

```bash
pm2 logs ue-assets-backend --lines 100
```

### 3. 测试后端登录

```bash
curl -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@admin.local","password":"admin123"}' \
  -v
```

### 4. 测试 /me 接口

```bash
# 先获取 token
TOKEN=$(curl -s -X POST https://api.factory-buy.com/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@admin.local","password":"admin123"}' \
  | jq -r '.token')

# 测试 /me
curl https://api.factory-buy.com/me \
  -H "Authorization: Bearer $TOKEN" \
  -v
```

---

## 📝 常见错误

### 错误 1: 401 Unauthorized

**原因：** 密码不匹配或 email 格式不匹配

**解决：** 检查 `USER_WHITELIST` 和 `ADMIN_USERS` 配置

### 错误 2: 404 Not Found

**原因：** 后端服务未启动或路由未注册

**解决：** 
1. 检查后端服务状态：`pm2 list`
2. 检查路由注册：确保 `MeController` 在 `AuthModule` 中
3. 重启服务：`pm2 restart ue-assets-backend --update-env`

### 错误 3: CORS 错误

**原因：** 前端域名不在后端 CORS 允许列表中

**解决：** 检查后端 `FRONTEND_URL` 配置

---

## 🎯 推荐配置

### 开发环境

**前端 `.env.local`：**
```env
NEXTAUTH_SECRET=dev-secret-key-32-chars-minimum
NEXTAUTH_URL=http://localhost:3000
ADMIN_USERS=admin:admin123
BACKEND_TEST_PASSWORD=admin123
NEXT_PUBLIC_BACKEND_API_URL=http://localhost:3001
```

**后端 `.env`：**
```env
JWT_SECRET=dev-secret-key-32-chars-minimum
NEXTAUTH_SECRET=dev-secret-key-32-chars-minimum
USER_WHITELIST=admin@admin.local:admin123
MODEL_ENABLED=false
BILLING_ENABLED=false
INITIAL_CREDITS=100
PORT=3001
FRONTEND_URL=http://localhost:3000
```

### 生产环境

**前端（Vercel）：**
```env
NEXTAUTH_SECRET=production-secret-key-32-chars-minimum
NEXTAUTH_URL=https://your-domain.com
ADMIN_USERS=admin@factory-buy.com:secure-password
BACKEND_TEST_PASSWORD=secure-password
NEXT_PUBLIC_BACKEND_API_URL=https://api.factory-buy.com
```

**后端（服务器）：**
```env
JWT_SECRET=production-secret-key-32-chars-minimum
NEXTAUTH_SECRET=production-secret-key-32-chars-minimum
USER_WHITELIST=admin@factory-buy.com:secure-password
MODEL_ENABLED=false
BILLING_ENABLED=false
INITIAL_CREDITS=1000
FRONTEND_URL=https://your-domain.com
```

