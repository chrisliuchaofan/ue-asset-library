...
如果想在**两台电脑上同时开发不同的功能**，可以使用 Git 分支：

### 工作流程示例

**场景**：家里电脑开发功能A，公司电脑开发功能B
...
# 2. 创建并切换到新分支
git checkout -b feature/home-feature-a
...
git add -A
git commit -m "完成功能A"
git push origin feature/home-feature-a
```
...
# 2. 创建并切换到新分支
git checkout -b feature/company-feature-b
...
git add -A
git commit -m "完成功能B"
git push origin feature/company-feature-b
```
...
# 3. 合并功能A分支
git merge feature/home-feature-a

# 4. 合并功能B分支
git merge feature/company-feature-b
...
3. **推送分支**：
   - 源代码管理 → 提交 → 推送
   - 首次推送新分支时，Cursor 会提示设置上游分支

4. **合并分支**：
   - 切换到 `main` 分支
   - 命令面板（`Cmd + Shift + P`）→ `Git: Merge Branch`
   - 选择要合并的分支
